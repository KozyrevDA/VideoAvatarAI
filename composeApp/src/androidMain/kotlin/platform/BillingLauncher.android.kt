...
import billing.RuStoreBillingProvider
        ...
        object : BillingLauncher {
            override fun launch(productId: String, onResult: (Boolean, String?) -> Unit) {
                CoroutineScope(Dispatchers.Main).launch {
                    // Пробуем RuStore сначала, затем Google Play
                    val rustore = RuStoreBillingProvider()
                    val provider = if (rustore.isAvailable()) rustore else GooglePlayBillingProvider()
                    when (val result = provider.purchase(activity, productId)) {
